...
class IntrinsicDistance(VisualizationInterface):
    ...
    def _are_neighbours(self, x: Tuple[int, int], y: Tuple[int, int]) -> bool:
        return abs(x[0] - y[0]) == 1 and abs(x[1] - y[1]) == 0 or abs(x[0] - y[0]) == 0 and abs(x[1] - y[1]) == 1

    def _cost_to_move(self, x: Tuple[int, int], y: Tuple[int, int], distances: np.ndarray) -> float:
        return distances[y]
