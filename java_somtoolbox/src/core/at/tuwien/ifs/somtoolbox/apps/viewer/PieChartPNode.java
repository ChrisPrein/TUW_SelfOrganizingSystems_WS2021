 ...
 * @version $Id: PieChartPNode.java 4109 2011-02-05 21:54:36Z mayer $
 */
public class PieChartPNode extends PNode {
    public enum PieChartLabelMode {
        None, Count, Percent, ClassName
    }
    ...
    private final ClassNameLabeler classNameLabeler = new ClassNameLabeler();
    ...
    public PieChartPNode(double x, double y, double w, double h, int[] values, String[] classNames, int itemCount) {
        ...
        defaultLabelFont = plot.getLabelFont();
        smallLabelFont = UiUtils.scaleFont(defaultLabelFont, 0.75);
    ...
    public void setShowLegend(PieChartLabelMode mode) {
        switch (mode) {
            case Count:
                ...
                plot.setLabelFont(defaultLabelFont);
                break;
            case Percent:
                ...
                break;
            case ClassName:
                plot.setLabelGenerator(classNameLabeler);
                plot.setLabelFont(smallLabelFont);
                break;
            case None:
    ...
    private class CountLabeler implements PieSectionLabelGenerator {
        ...
        public String generateSectionLabel(PieDataset dataset, Comparable key) {
            if (getLegendColor(dataset.getIndex(key)).getAlpha() == 0) {
                return null;
            }
    ...
    @SuppressWarnings("rawtypes")
    private class ClassNameLabeler implements PieSectionLabelGenerator {
        @Override
        public String generateSectionLabel(PieDataset dataset, Comparable key) {
            if (getLegendColor(dataset.getIndex(key)).getAlpha() == 0) {
                return null;
            }
            return classNames[dataset.getIndex(key)];
        }
