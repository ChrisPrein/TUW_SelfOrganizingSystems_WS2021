 ...
 * 
 * @author Jakob Frank
 * @version $Id: SOMToolboxMain.java 4226 2011-10-12 13:07:38Z mayer $
 */
public class SOMToolboxMain {
    ...
    public static void main(String[] args) {
        ...
        if (result.getBoolean("listmains")) {
            ...
            // make a clean copy of the arguments for the final main class
            String[] cleanArgs = Arrays.copyOfRange(args, 0, args.length);
            // 1. remove the --gui argument
            if (useGUI) {
                String longFlag = "--" + guiSwitch.getLongFlag();
                if (ArrayUtils.indexOf(cleanArgs, longFlag) != ArrayUtils.INDEX_NOT_FOUND) {
                    cleanArgs = (String[]) ArrayUtils.remove(cleanArgs, ArrayUtils.indexOf(cleanArgs, longFlag));
                } else {
                    String shortFlag = "-" + guiSwitch.getShortFlag();
                    cleanArgs = (String[]) ArrayUtils.remove(cleanArgs, ArrayUtils.indexOf(cleanArgs, shortFlag));
                }
            }
            // 2. remove the mainClass argument
            if (ArrayUtils.indexOf(cleanArgs, mainClass) != ArrayUtils.INDEX_NOT_FOUND) {
                cleanArgs = (String[]) ArrayUtils.remove(cleanArgs, ArrayUtils.indexOf(cleanArgs, mainClass));
            }
