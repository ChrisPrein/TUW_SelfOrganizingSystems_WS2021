 ...
 * @author Jakob Frank
 * @author Rudolf Mayer
 * @version $Id: SOMTrainer.java 4307 2014-01-09 12:31:37Z mayer $
...
public class SOMTrainer extends JFrame implements SOMToolboxApp {
    ...
    public static final SOMToolboxApp.Type APPLICATION_TYPE = SOMToolboxApp.Type.Training;
    ...
    private Properties createSOMProps() {
        ...
        p.setProperty(FileProperties.OPTION_OUTPUT_DIRECTORY.getID(), getTxtOutDir().getText());
        p.setProperty(FileProperties.OPTION_WORKING_DIRECTORY.getID(), getTxtOutDir().getText());
        p.setProperty(FileProperties.OPTION_NAME_PREFIX.getID(), getTxtTitle().getText());
        p.setProperty(FileProperties.OPTION_INPUT_VECTOR.getID(), getTxtInputVecotrFile().getText());
        p.setProperty(FileProperties.OPTION_SPARSE_DATA.getID(), getChkSparse().isSelected() ? "yes" : "no");
        p.setProperty(FileProperties.OPTION_NORMALIZED.getID(), getChkNormalized().isSelected() ? "yes" : "no");
        if (getTxtTemplateVecotrFile().getText().length() > 0) {
            p.setProperty(FileProperties.OPTION_TEMPLATE_VECTOR.getID(), getTxtTemplateVecotrFile().getText());
        }

        p.setProperty(SOMProperties.OPTION_XSIZE.getID(), getSpnXSize().getValue().toString());
        p.setProperty(SOMProperties.OPTION_YSIZE.getID(), getSpnYSize().getValue().toString());
        if (getChkLernrate().isSelected()) {
            p.setProperty(SOMProperties.OPTION_LEARNRATE.getID(), getSpnLearnrate().getValue().toString());
        }
        if (getChkSigma().isSelected()) {
            p.setProperty(SOMProperties.OPTION_SIGMA.getID(), getSpnSigma().getValue().toString());
        }

        p.setProperty(FileProperties.OPTION_RANDOM_SEED.getID(), getSpnRandomSeed().getValue().toString());

        ClassComboBoxModel<DistanceMetric> mm = getCmbMetricModel();
        if (mm.getSelectedClass() != null) {
            p.setProperty(SOMProperties.OPTION_METRIC_NAME.getID(), mm.getSelectedClass().getName());
        }

        ClassComboBoxModel<QualityMeasure> qm = getCmbQualityMeasureModel();
        if (qm.getSelectedClass() != null) {
            p.setProperty(SOMProperties.OPTION_GROWTH_QUALITY.getID(), qm.getSelectedClass().getName());
        }

        if (rdoIterations.isSelected()) {
            p.setProperty(SOMProperties.OPTION_NUM_ITERATIONS.getID(), getSpnIterations().getValue().toString());
        } else {
            p.setProperty(SOMProperties.OPTION_NUM_CYCLES.getID(), getSpnIterations().getValue().toString());
        }
