...
import at.tuwien.ifs.somtoolbox.visualization.clustering.DendrogramView;
 ...
 * @author Rudolf Mayer
 * @version $Id: ClusteringControl.java 4353 2015-03-10 16:33:57Z mayer $
 */
public class ClusteringControl extends AbstractViewerControl {
    ...
    private DendrogramView dendogramView;
    ...
    public void init() {
        ...
        final JComboBox<KMeans.InitType> initialisationBox = new JComboBox<KMeans.InitType>(KMeans.InitType.values());
    ...
    private void updateDendogram() {
        TreeBuilder tb = mapPane.getMap().getClusteringTreeBuilder();
        if (!(tb instanceof NonHierarchicalTreeBuilder)) {
            ClusteringTree t = mapPane.getMap().getClusteringTree();

            if (t != null) {
                getDendogramView().update(t, numClusters);
    ...
    private DendrogramView getDendogramView() {
        if (dendogramView == null) {
            dendogramView = new DendrogramView(maxCluster, spinnerNoCluster);
        }
