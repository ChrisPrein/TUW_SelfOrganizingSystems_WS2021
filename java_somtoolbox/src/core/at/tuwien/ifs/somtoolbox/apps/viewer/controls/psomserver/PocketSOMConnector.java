...
/**
 * @author Jakob Frank
 * @version $Id: PocketSOMConnector.java 4220 2011-08-16 12:15:09Z mayer $
 */
        ...
        private class Worker extends Thread {// implements Runnable {
            ...
            private void sendStatusInfo(String line, BufferedReader in, PrintStream out) {
                ...
                if ("CurrentSong".equalsIgnoreCase(ls[1])) {
                    ...
                    out.println("EndList");
                    // start add by johannes duneser
                } else if ("MapType".equalsIgnoreCase(ls[1])) {
                    int mapDim = state.growingLayer.getDim();
                    String mapType = "RP"; // default
                    switch (mapDim) {
                        case 1380:
                            mapType = "RP";
                            break;
                    ...
                    String responseString = "INFO: type=" + mapType + "\n";
                    try {
                        out.write(responseString.getBytes());
                    } catch (IOException e) {
                        System.err.printf("Failed to write Response to output stream", line);
                        return;
                    }

                } else if ("Winner".equalsIgnoreCase(ls[1])) {
                    Vector<Double> featuresVector = new Vector<Double>();

                    while (true) {
                        String vectorLine = null;
                        try {
                            vectorLine = in.readLine();
                        } catch (IOException e) {

                        }
                        if (vectorLine == null || vectorLine.equalsIgnoreCase("EndGetWinner")) {
                            break;
                        }

                        String[] values = vectorLine.replaceAll("\\n", "").split("\\s+");
                        int size = 0;
                        for (String value : values) {
                            size++;
                            try {
                                featuresVector.add(new Double(Double.parseDouble(value)));
                            } catch (NumberFormatException e) {
                                System.err.printf("Failed to parse feature vector values!");
                                System.err.printf("value-str: [" + value + "]");
                                return;
                            }
                        }
                    ...
                    if (featuresVector.size() < 1) {
                        System.err.printf("Wrong feature vetor");
                        return;
                    }

                    double[] fVect = new double[featuresVector.size()];
                    for (int i = 0; i < featuresVector.size(); i++) {
                        fVect[i] = featuresVector.get(i);
                    }

                    InputDatum inputDatum = new InputDatum("mapRecording", fVect);
                    Unit winner = state.growingLayer.getWinner(inputDatum);

                    String reponseString = "INFO: winner=" + winner.getXPos() + "," + winner.getYPos() + ","
                            + winner.getZPos() + "\n";
                    try {
                        out.write(reponseString.getBytes());
                    } catch (IOException e) {
                        System.err.printf("Failed to write Response to output stream", line);
                        return;
                    }

                    // end add by johannes duneser
                } else {
                    System.err.printf("Unknown request: \"%s\"%n", line);
                }
            }
