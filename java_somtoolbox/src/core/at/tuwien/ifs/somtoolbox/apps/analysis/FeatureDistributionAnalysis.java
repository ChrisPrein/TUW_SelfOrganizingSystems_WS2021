...
/**
 * @author Rudolf Mayer
 * @version $Id: FeatureDistributionAnalysis.java 4111 2011-02-05 21:57:00Z mayer $
 */
public class FeatureDistributionAnalysis {
    ...
    public static void main(String[] args) throws SOMToolboxException {
        ...
        File inputVectorFileName = config.getFile("inputVectorFile");
        File classInfoFileName = config.getFile("classInformationFile");

        final int paddingLength = 9;

        SOMLibSparseInputData input = new SOMLibSparseInputData(inputVectorFileName.getPath());
        ...
        for (int i = 0; i < classNames.length; i++) {
            ...
            double[][] classData = input.getData(className);
            means[i] = VectorTools.meanVector(classData);
            ...
            for (int j = 0; j < variances[i].length; j++) {
                variances[i][j] = StatUtils.variance(VectorTools.slice(classData, j));
            }
        }
