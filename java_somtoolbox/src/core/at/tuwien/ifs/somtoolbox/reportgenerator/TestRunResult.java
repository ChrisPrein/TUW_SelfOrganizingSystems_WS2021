 ...
 * @version $Id: TestRunResult.java 4342 2015-03-06 15:56:51Z mayer $
 */
public class TestRunResult {
    ...
    public MapQEContainer getMapMQE() {
        ...
        try {
            qeContainer = new MapQEContainer(qe.getMapQuality(QuantizationError.MQE_MAP));
    ...
    public UnitQEContainer getMinUnitMQE() {
        ...
        try {
            errors = qe.getUnitQualities(QuantizationError.MQE_UNIT);
            minError = errors[0][0];
            argminx.add(new Integer(0));
            argminy.add(new Integer(0));
            for (int i = 0; i < errors.length; i++) {
    ...
    public double getMQEForUnit(int x, int y) {
        try {
            return this.somQE().getUnitQualities(QuantizationError.MQE_UNIT)[x][y];
        } catch (QualityMeasureNotFoundException e) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning(
    ...
    public void fillQMContainer() throws QualityMeasureNotFoundException {
        ...
        double[][] qerrUnits = qerr.getUnitQualities(QuantizationError.QE_UNIT);
        double[][] mqerrUnits = qerr.getUnitQualities(QuantizationError.MQE_UNIT);
        ...
        if (wanted_QM.contains("Mean Quantization Error")) {
            this.QMContainer.putMapQualities("Mean Quantization Error", qerr.getMapQuality(QuantizationError.MQE_MAP));
        }
        if (wanted_QM.contains("Quantization Error")) {
            this.QMContainer.putMapQualities("Quantization Error", qerr.getMapQuality(QuantizationError.QE_MAP));
        }
