 ...
 * @version $Id: SOMLibSparseInputData.java 4293 2014-01-08 15:38:45Z mayer $
 */
public class SOMLibSparseInputData extends AbstractSOMLibSparseInputData {
    ...
    public void setValue(int datumIndex, int attributeIndex, double value) {
        data.set(datumIndex, attributeIndex, value);
    }
    ...
    protected void readVectorFile(String vectorFileName, boolean sparse) {
        ...
        readVectorFile(sparse, br);
    }

    private void readVectorFile(boolean sparse, BufferedReader br) {
        String line = null;
        int lineNumber = 0;
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (header.equals("$TYPE")) {
                    ...
                    if (!contentType.isSparse()) {
                        Logger.getLogger("at.tuwien.ifs.somtoolbox").info(
                                "Not sparse content type ('" + contentType + "'). Setting sparsity to false.");
                        sparse = false;
                    }
    ...
    public DoubleMatrix2D getDataAsMatrix() {
        return data;
    }

    /**
     * Parses the given contents in SOMLib format to a {@link SOMLibSparseInputData} object.<br/>
     ...
     */
    public static SOMLibSparseInputData parse(String contents) {
        SOMLibSparseInputData data = new SOMLibSparseInputData();
        data.init(DEFAULT_SPARSE, DEFAULT_NORMALISED, DEFAULT_RANDOM_SEED);
        data.readVectorFile(DEFAULT_SPARSE, new BufferedReader(new StringReader(contents)));
        return data;
    }
