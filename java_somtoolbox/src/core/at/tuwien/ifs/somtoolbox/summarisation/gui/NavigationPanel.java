 ...
 * @author Rudolf Mayer
 * @version $Id: NavigationPanel.java 4135 2011-02-08 13:14:18Z mayer $
 */
public class NavigationPanel extends JPanel {
    ...
    private TitledCollapsiblePanel palettePanel = null;

    private CommonSOMViewerStateData state;

    public NavigationPanel(SummariserGUI summariserGUI, CommonSOMViewerStateData state, Object[] itemNames) {
        super(new GridBagLayout());
        this.summarizer = summariserGUI;
        ...
        documentdispl = new DocumentDisplayer(state, summariserGUI.scrollP, scorer.getParsedDocuments(), itemNames);

        PaletteSelectionPanel paletteSelectionPanel = new PaletteSelectionPanel(SummariserGUI.palettes);
        paletteSelectionPanel.addPaletteChangeListener(new PaletteChangeListener() {
            @Override
            public void paletteChanged(Palette palette) {
                documentdispl.setPalette(palette.getColors());
            }
        });
        palettePanel = new TitledCollapsiblePanel("Palette Selection");
        palettePanel.add(paletteSelectionPanel);

        doclistPanel = new DocumentListPanel(this);
        searchPanel = new SearchPanel(this);
        optionPanel = new OptionPanel(this);

        documentdispl.setPalette(paletteSelectionPanel.getSelectedPalette().getColors());
