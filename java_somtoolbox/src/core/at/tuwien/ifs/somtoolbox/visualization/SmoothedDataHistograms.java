 ...
 * @author Rudolf Mayer
 * @version $Id: SmoothedDataHistograms.java 4161 2011-02-11 16:23:39Z mayer $
 */
public class SmoothedDataHistograms extends AbstractMatrixVisualizer implements BackgroundImageVisualizer,
    ...
    protected String getVisualisationSpecificCacheKey(int currentVariant) {
        return super.getVisualisationSpecificCacheKey(currentVariant) + CACHE_KEY_SECTION_SEPARATOR + "smoothing:" + s;
    }
    ...
    public BufferedImage createVisualization(int index, GrowingSOM gsom, int width, int height, int blockWidth,
        ...
        return super.createImage(gsom, smoothingCache[index].get(s).mh, width, height, isInterpolate());
    }
    ...
    protected BiCubicSplineFast computeSpline(GrowingSOM gsom, DoubleMatrix2D matrix, int width, int height,
        ...
        // top-right corner
        sdhWithBorders.setQuick(0, xSize + 1, matrix.getQuick(0, xSize - 1)
                - (matrix.getQuick(1, xSize - 2) - matrix.getQuick(0, xSize - 1)) / 2);
        // bottom-left corner
        sdhWithBorders.setQuick(ySize + 1, 0, matrix.getQuick(ySize - 1, 0)
                - (matrix.getQuick(ySize - 2, 1) - matrix.getQuick(ySize - 1, 0)) / 2);
        // FIXME ? was sdhWithBorders.setQuick(xSize, ySize) for former method 'createVisualization3'.
        // bottom-right corner
        sdhWithBorders.setQuick(ySize + 1, xSize + 1, matrix.getQuick(ySize - 1, xSize - 1)
                - (matrix.getQuick(ySize - 2, xSize - 2) - matrix.getQuick(ySize - 1, xSize - 1)) / 2);

        for (int x = 1; x < xSize + 1; x++) {
            // top row
            sdhWithBorders.setQuick(0, x, matrix.getQuick(0, x - 1)
                    - (matrix.getQuick(1, x - 1) - matrix.getQuick(0, x - 1)) / 2);
            // bottom row
            sdhWithBorders.setQuick(ySize + 1, x, matrix.getQuick(ySize - 1, x - 1)
                    - (matrix.getQuick(ySize - 2, x - 1) - matrix.getQuick(ySize - 1, x - 1)) / 2);
        }
        for (int y = 1; y < ySize + 1; y++) {
            // left column
            sdhWithBorders.setQuick(y, 0, matrix.getQuick(y - 1, 0)
                    - (matrix.getQuick(y - 1, 1) - matrix.getQuick(y - 1, 0)) / 2);
            // right column
            sdhWithBorders.setQuick(y, xSize + 1, matrix.getQuick(y - 1, xSize - 1)
