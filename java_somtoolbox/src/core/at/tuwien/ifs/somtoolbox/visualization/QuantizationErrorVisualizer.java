 ...
 * @version $Id: QuantizationErrorVisualizer.java 4342 2015-03-06 15:56:51Z mayer $
 */
        ...
        try {
            for (int j = 0; j < gsom.getLayer().getYSize(); j++) {
                for (int i = 0; i < gsom.getLayer().getXSize(); i++) {
                    ...
                    if (u.getNumberOfMappedInputs() > 0) {
                        final double value = qe.getUnitQualities(QuantizationError.MQE_UNIT)[u.getXPos()][u.getYPos()];
                        if (value > maxMQE) {
                            maxMQE = value;
                        }
                        if (value < minMQE) {
        ...
        try {
            for (int y = 0; y < gsom.getLayer().getYSize(); y++) {
                for (int x = 0; x < gsom.getLayer().getXSize(); x++) {
                    ...
                    if (u.getNumberOfMappedInputs() > 0) {
                        ci = (int) Math.round((qe.getUnitQualities(QuantizationError.MQE_UNIT)[u.getXPos()][u.getYPos()] - minMQE)
