 ...
 * @version $Id: SOMLibMapOutputter.java 4342 2015-03-06 15:56:51Z mayer $
 */
                                ...
                                try {
                                    bw.write("$QUANTERROR_UNIT "
                                            + u.getLayer().getQualityMeasure().getUnitQualities(
                                                    QuantizationError.QE_UNIT)[u.getXPos()][u.getYPos()]);
                                    bw.newLine();
                                    bw.write("$QUANTERROR_UNIT_AVG "
                                            + u.getLayer().getQualityMeasure().getUnitQualities(
                                                    QuantizationError.MQE_UNIT)[u.getXPos()][u.getYPos()]);
                                    bw.newLine();
        ...
        while (maps.size() > 0) { // process all remaining maps
            ...
            try {
                ...
                mapDescription.setProperty(SOMLibMapDescription.URL_DATA_WINNER_MAPPING,
                        getDataWinnerMappingFileName(fDir, fName, gsom.getLayer().getIdString(), false));
