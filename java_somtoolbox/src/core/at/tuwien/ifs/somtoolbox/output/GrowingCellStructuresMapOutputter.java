 ...
 * 
 * @author Johannes Infuehr
 * @author Andreas Zweng
 * @version $Id: GrowingCellStructuresMapOutputter.java 4343 2015-03-07 22:02:29Z mayer $
 */
                            ...
                            if (u.getLayer().getQualityMeasure().getClass().getName().equals(
                                ...
                                try {
                                    bw.write("$QUANTERROR_UNIT "
                                            + u.getLayer().getQualityMeasure().getUnitQualities(
                                                    QuantizationError.QE_UNIT)[i][j]);
                                    bw.newLine();
                                    bw.write("$QUANTERROR_UNIT_AVG "
                                            + u.getLayer().getQualityMeasure().getUnitQualities(
                                                    QuantizationError.MQE_UNIT)[i][j]);
                                    bw.newLine();
        ...
        while (maps.size() > 0) { // process all remaining maps
            ...
            try {
                ...
                try {
                    ...
                    mapDescription.setProperty(SOMLibMapDescription.QUANT_ERROR_VECTOR,
                            mapQuality / inputData.numVectors());
                } catch (QualityMeasureNotFoundException e) {
                    mapDescription.setProperty(SOMLibMapDescription.QUANT_ERROR_MAP,
                            "<error receiving value: " + e.getMessage() + ">");
                    mapDescription.setProperty(SOMLibMapDescription.QUANT_ERROR_VECTOR,
                            "<error receiving value: " + e.getMessage() + ">");// TODO
                    e.printStackTrace();
                }
                ...
                mapDescription.setProperty(SOMLibMapDescription.URL_WEIGHT_VECTOR,
                        getWeightVectorFileName(fDir, fName, idString, false));
                ...
                mapDescription.setProperty(SOMLibMapDescription.URL_UNIT_DESCRIPTION,
                        getUnitDescriptionFileName(fDir, fName, idString, false));
                mapDescription.setProperty(SOMLibMapDescription.URL_TEMPLATE_VECTOR, fileProps.templateFileName(true));
                mapDescription.setProperty(SOMLibMapDescription.URL_DATA_WINNER_MAPPING,
                        getDataWinnerMappingFileName(fDir, fName, csommap.getLayer().getIdString(), false));
