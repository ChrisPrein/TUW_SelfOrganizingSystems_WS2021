 ...
 * @version $Id: TrajectoryOutputter.java 4363 2015-03-11 17:30:06Z mayer $
 */
        ...
        for (int j = 0; j < gsom.getLayer().getYSize(); j++) {
            for (int i = 0; i < gsom.getLayer().getXSize(); i++) {
                ...
                if (nrDataNames[i][j] > 0) {
                    ...
                    g.setPaint(bg);
                    g.fill(new Rectangle2D.Double(i * unitWidth + 3, j * unitHeight + 3, unitWidth - 1, unitHeight - 1));
                } else {
                    // g.setPaint(Color.white);
                    // System.out.println("x1:"+((i*unitWidth)+1)+", y1:"+((j*unitHeight)+1)+", x2:"+((i*unitWidth)+unitWidth)+",
                    // y2:"+((j*unitHeight)+unitHeight));
                    g.draw(new Rectangle2D.Double(i * unitWidth + 3, j * unitHeight + 3, unitWidth - 1, unitHeight - 1));
